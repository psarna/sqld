...
impl Connection for GenericConnection {
    async fn batch(
        ...
        match self {
            ...
            #[cfg(feature = "spin_backend")]
            Self::Spin(w) => w.batch(stmts).await,
        }
    }
}
...
pub fn connect() -> anyhow::Result<GenericConnection> {
    ...
    Ok(match backend.as_str() {
        ...
        #[cfg(feature = "spin_backend")]
        "spin" => {
            anyhow::bail!("Connecting from spin API may need access to worker::RouteContext. Please call libsql_client::spin::Connection::connect_from_url() directly")
        },
