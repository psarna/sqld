...
impl<'a> Connection<'a> {
    ...
    fn execute_query(
        ...
        // FIXME: why does execute() return Option<Rows> instead of Result<Option<Rows>>?
        let mut qresult = stmt
            .execute(&libsql::Params::None)
            .ok_or(Error::Internal("Query excecution failed".to_string()))?;
        builder.begin_rows()?;
